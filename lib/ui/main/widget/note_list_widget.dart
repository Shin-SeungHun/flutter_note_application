...
class _NoteListWidgetState extends State<NoteListWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> generatedChildren = mainViewModel.noteList.asMap().entries.map((entry) {
      ...
      return Padding(
        ...
        child: InkWell(
          ...
          child: Card(
            color: (Commons.convertIndexToColor(index: item.color.toString()) as Color).withOpacity(0.8),
            child: Container(
                            ...
                            child: IconButton(
                              onPressed: () async {
                                bool? isConfirm = await Commons.onAlertDialog(
                                  context: context,
                                  title: '삭제',
                                  content: '"${item.title}"을/를 삭제 하시겠습니까?',
                                );
    ...
    return ReorderableBuilder(
      ...
      onReorder: (List<OrderUpdateEntity> orderUpdateEntities) {
        for (final orderUpdateEntity in orderUpdateEntities) {
          final item = mainViewModel.noteList.removeAt(orderUpdateEntity.oldIndex);
